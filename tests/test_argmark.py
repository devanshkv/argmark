...
def test_main_file_not_found_error_handling(caplog):
    ...
    for record in caplog.records:
        if record.levelname == "ERROR" and non_existent_file in record.message:
            ...
            if "File not found" in record.message or "No such file or directory" in record.message:
                found_log = True
                break
    assert found_log, f"Expected error message for {non_existent_file} not found in logs: {caplog.text}"
...
def test_gen_help_with_subparsers():
    ...
    assert filecmp.cmp(md_file, answer_file, shallow=False), f"Generated {md_file} does not match {answer_file}."
